    ...
    ObservingService observingService) : PageModel
{
    ...
    public int UsersToCount { get; set; }
    ...
    public async Task<IActionResult> OnGetAsync()
    {
        ...
        try
        {
            Observings = await _observingService.GetUsersToById(UserId);
            UsersToCount = await _observingService.GetUsersToCount(UserId);
        }
        catch (Exception ex)
        {
            TempData[Messages.ERROR] = "Ошибка при загрузке подписчиков: " + ex.Message;
        }
